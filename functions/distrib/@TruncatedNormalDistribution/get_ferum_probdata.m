...
probdata.marg = [               ...
    51 * ones(dim, 1)           ...  % gaussian distrib
    nan(dim, 2)                 ...  % mean, std: will be computed
    distrib.mu(:)               ...  % starting point
    distrib.mu(:) distrib.sd(:) ...  % mean, std of the untruncated distrib
    distrib.xL(:) distrib.xU(:) ...  % lower and upper bounds
