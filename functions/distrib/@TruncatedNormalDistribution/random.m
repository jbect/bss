...
function x = random (TN, samplesize)

% TODO: use inversion sampling instead (?)

N = NormalDistribution(TN.mu, TN.sd);

x = random(N, samplesize);
b = ~ is_inside_support(TN, x);
r = sum(b);
